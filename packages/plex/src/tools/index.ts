...
export function registerTools(server: McpServer, service: Service) {
  ...
  server.tool(
    ...
    async (params, _extra: RequestHandlerExtra): Promise<ToolResponse> => {
      try {
  ...
  server.tool(
    ...
    async (args: {
      ...
      timeout?: number;
      async?: boolean;
    }, _extra: RequestHandlerExtra): Promise<ToolResponse> => {
      try {
  ...
  server.tool(
    ...
    async ({ id, includeDetails }, _extra) => {
      try {
  ...
  server.tool(
    ...
    async ({ timeout, includeMetrics }, _extra) => {
      try {
